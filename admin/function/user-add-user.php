...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
   ...
   if (!empty($last_name) && !empty($first_name) && !empty($middle_name) && !empty($gender) && !empty($contact) && !empty($email_address) && !empty($profile_picture) && !empty($birthday) && !empty($username) && !empty($password) && !empty($account_status)) {
      if ($account_status === "Member") {
         ...
         $activity = "$uname Added $fullname in member";
         ...
         mysqli_query($con, $query);
         $activity = "$uname Added $fullname in Admin";
         $log = "INSERT INTO `acti-log` (date,activity) values ('$date','$activity')";
         mysqli_query($con, $log);
         echo '<script>alert("account successfully added");
         window.location.href=" ../member.php"</script>';
         ...
         mysqli_query($con, $query);
         $activity = "$uname Added $fullname in Junkshop";
         $log = "INSERT INTO `acti-log` (date,activity) values ('$date','$activity')";
         mysqli_query($con, $log);
         echo '<script>alert("account successfully added");
         window.location.href=" ../member.php"</script>';
                        ...
                        <div class="row">
                           <div class="col-6">
                              <button type="submit" class="btn btn-block"
                                 style="background-color: rgba(28,153,84);color: rgb(235,235,235)" name="ADDUSER">ADD
                                 USER</button>
                           </div>
                           <div class="col-6">
